...
<html lang="en">
    <head>
        ...
        <meta property="og:title" content="<?php echo htmlentities($row['title']);?>"/>
        <meta property="og:description"content="<?php echo htmlentities($row['description']);?>"/>
        <meta property="og:image" content="<?php echo htmlentities($row['image']);?>"/>
            ...
            <div class="pt-3 bg-white">
                <div class="container post-content">
                    <div class="row">
	                    ...
	                    $query = mysqli_query($kon, "SELECT post.image, post.title, post.PostingDate AS postingdate, LEFT(post.description, 100) AS description, post.url FROM post WHERE title LIKE '%".$cari."%' AND Is_Active=1");
	                    if ($query === FALSE) {
                            die(mysqli_error());
                        } else {
